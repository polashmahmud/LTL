    ...
    @forelse($menu->items as $item)
        <li class="dd-item my-3" data-id="{{ $item->id }}">
            <div style="text-align: end; margin-bottom: -50px; margin-right: 15px">
                <a href="{{ route('settings.menus.builder.edit', [$menu, $item]) }}" class="btn btn-sm">Edit</a>
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col">
                                            <a class="btn btn-danger w-100"
                                               href="{{ route('settings.menus.builder.destroy', [$menu, $item]) }}"
                                            ...
                                            <form id="delete-form-{{ $item->id }}"
                                                  action="{{ route('settings.menus.builder.destroy', [$menu, $item]) }}"
                                                  method="POST"
                                                  class="d-none">
                                                @csrf
                                                @method('DELETE')
                ...
                <ol class="dd-list">
                    @foreach($item->children as $children)
                        <li class="dd-item" data-id="{{ $children->id }}">
                            <div style="text-align: end; margin-bottom: -50px; margin-right: 15px">
                                <a href="{{ route('settings.menus.builder.edit', [$item->menu->id, $children]) }}"
                                ...
                                <div class="modal modal-blur fade" id="modal-danger-{{ $children->id }}" tabindex="-1"
                                     role="dialog"
                                     aria-hidden="true">
                                    <div class="modal-dialog modal-sm modal-dialog-centered" role="document">
                                        <div class="modal-content">
                                            ...
                                            <div class="modal-body text-center py-4">
                                                ...
                                                <h3>Are you sure?</h3>
                                                <div class="text-muted">Do you really want to remove this menu? What
                                                    you've done cannot
                                                    be undone.
                                                </div>
                                            </div>
                                            <div class="modal-footer">
                                                <div class="w-100">
                                                    <div class="row">
                                                        ...
                                                        <div class="col">
                                                            <a class="btn btn-danger w-100"
                                                               href="{{ route('settings.menus.builder.destroy', [$item, $children]) }}"
                                                               onclick="event.preventDefault();
                                                     document.getElementById('delete-form-{{$children->id}}').submit();">
                                                                Delete
                                                            </a>

                                                            <form id="delete-form-{{ $children->id }}"
                                                                  action="{{ route('settings.menus.builder.destroy', [$item, $children]) }}"
                                                                  method="POST"
                                                                  class="d-none">
                                                                @csrf
                                                                @method('DELETE')
                            ...
                            <div class="dd-handle my-3">
                                <div>
                                    ...
                                    <small class="url">{{ $children->url }}</small>
                                </div>
                            </div>
                        </li>
                    @endforeach
