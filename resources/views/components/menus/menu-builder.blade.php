                            ...
                            <div class="modal-footer">
                                <div class="w-100">
                                    <div class="row">
                                        ...
                                        <div class="col">
                                            <a class="btn btn-danger w-100" href="{{ route('settings.menus.builder.destroy', [$menu, $item]) }}"
                                               onclick="event.preventDefault();
                                                     document.getElementById('delete-form-{{$item->id}}').submit();">
                                                Delete
                                            </a>

                                            <form id="delete-form-{{ $item->id }}" action="{{ route('settings.menus.builder.destroy', [$menu, $item]) }}" method="POST"
                                                  class="d-none">
                                                @csrf
                                                @method('DELETE')
                                            </form>
                ...
                <ol class="dd-list">
                    @foreach($item->children as $children)
                        <li class="dd-item" data-id="{{ $children->id }}">
                            <div style="text-align: end; margin-bottom: -50px; margin-right: 15px">
                                <a href="{{ route('settings.menus.builder.edit', [$item, $children]) }}" class="btn btn-sm">Edit</a>
                                <a href="#" class="btn btn-sm btn-danger" data-bs-toggle="modal"
                                            ...
                                            <div class="modal-footer">
                                                <div class="w-100">
                                                    <div class="row">
                                                        ...
                                                        <div class="col">
                                                            <a class="btn btn-danger w-100" href="{{ route('settings.menus.builder.destroy', [$item, $children]) }}"
                                                               onclick="event.preventDefault();
                                                     document.getElementById('delete-form-{{$children->id}}').submit();">
                                                                Delete
                                                            </a>

                                                            <form id="delete-form-{{ $children->id }}" action="{{ route('settings.menus.builder.destroy', [$item, $children]) }}" method="POST"
                                                                  class="d-none">
                                                                @csrf
                                                                @method('DELETE')
                                                            </form>
